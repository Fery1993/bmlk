<x-app-layout title="Invoice Baru" breadcrumb="Buat invoice jasa bongkar muat">
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm">
    <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
        ...
        <h2 class="text-sm font-semibold text-gray-700">Form Invoice Baru</h2>
    </div>

    <div class="p-5"
         x-data="{
            items: {{ old('items')
                ? Illuminate\Support\Js::from(collect(old('items'))->map(fn ($i) => [
                    'is_checked' => ! empty($i['is_checked']),
            ...
            volumeTon: {{ (float) old('volume_ton', 0) }},
            tarifPerTon: {{ (float) old('tarif_per_ton', 0) }},
            pakaiMaterai: {{ old('pakai_materai', true) ? 'true' : 'false' }},
        ...
        @if ($errors->any())
        <div class="mb-4 bg-red-50 border border-red-200 text-red-700 text-xs rounded-lg px-4 py-3">
        ...
        <form method="POST" action="{{ route('transaksi.invoices.store') }}">
            @csrf
            ...
            <div class="mb-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1.5">No. Invoice <span class="text-red-500">*</span></label>
                        <input type="text" name="nomor_invoice" value="{{ old('nomor_invoice', $nomorOtomatis) }}" required
                               placeholder="01/BMLK/VIII/2026"
                    ...
                    <div>
                        ...
                        <select name="customer_id" required
                            ...
                            <option value="{{ $c->id }}" @selected(old('customer_id') == $c->id)>{{ $c->nama }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="jatuh_tempo" value="{{ old('jatuh_tempo', today()->addDays(14)->format('Y-m-d')) }}" required
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="piutang_coa_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                            ...
                            <option value="{{ $c->id }}" @selected(old('piutang_coa_id') == $c->id)>{{ $c->kode_akun }} · {{ $c->nama_akun }}</option>
                            @endforeach
                        </select>
                        @error('piutang_coa_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        <select name="pendapatan_coa_id" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                            ...
                            <option value="{{ $c->id }}" @selected(old('pendapatan_coa_id') == $c->id)>{{ $c->kode_akun }} · {{ $c->nama_akun }}</option>
                            @endforeach
                        </select>
                        @error('pendapatan_coa_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
