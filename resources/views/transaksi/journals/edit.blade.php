<x-app-layout title="Edit Jurnal" breadcrumb="Ubah jurnal manual {{ $journal->nomor_jurnal }}">
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm">
    <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
        ...
        <h2 class="text-sm font-semibold text-gray-700">Form Edit Jurnal Manual</h2>
    </div>

    <div class="p-5"
         x-data="{
            rows: {{ Illuminate\Support\Js::from(
                old('items', $journal->items->map(fn ($i) => [
                    'coa_id' => $i->coa_id,
                    'debit' => (float) $i->debit,
                    'kredit' => (float) $i->kredit,
        ...
        <form method="POST" action="{{ route('transaksi.journals.update', $journal) }}">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-2 gap-4 mb-5">
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">No. Jurnal</label>
                    <input type="text" value="{{ $journal->nomor_jurnal }}" disabled
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $journal->tanggal->format('Y-m-d')) }}" required
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="keterangan" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">{{ old('keterangan', $journal->keterangan) }}</textarea>
                </div>
            </div>
