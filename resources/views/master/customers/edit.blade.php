<x-app-layout title="Edit Customer" breadcrumb="Ubah data customer {{ $customer->kode }}">

<x-form-card title="Form Edit Customer" :backRoute="route('master.customers.index')">
...
<form method="POST" action="{{ route('master.customers.update', $customer) }}">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" name="kode" value="{{ old('kode', $customer->kode) }}" required
        ...
        <div class="col-span-2">
            ...
            <textarea name="alamat" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">{{ old('alamat', $customer->alamat) }}</textarea>
            @error('alamat')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
    </div>

    <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
        <a href="{{ route('master.customers.index') }}" class="text-sm text-gray-500 hover:text-gray-700 px-4 py-2">Batal</a>
