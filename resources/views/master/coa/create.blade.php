<x-app-layout title="Akun Baru" breadcrumb="Tambah akun ke Chart of Account">

<x-form-card title="Form Akun Baru" :backRoute="route('master.coa.index')">

<form method="POST" action="{{ route('master.coa.store') }}">
    @csrf

    <div class="grid grid-cols-2 gap-4 mb-5">
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1.5">Kode Akun <span class="text-red-500">*</span></label>
            <input type="text" name="kode_akun" value="{{ old('kode_akun') }}" required placeholder="cth. 1101"
        ...
        <div>
            ...
            <input type="text" name="nama_akun" value="{{ old('nama_akun') }}" required placeholder="cth. Kas Besar"
                   class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
        ...
        <div>
            ...
            <select name="kelompok" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                ...
                <option value="aset"       @selected(old('kelompok')=='aset')>Aset</option>
                <option value="kewajiban"  @selected(old('kelompok')=='kewajiban')>Kewajiban</option>
                <option value="ekuitas"    @selected(old('kelompok')=='ekuitas')>Ekuitas</option>
                <option value="pendapatan" @selected(old('kelompok')=='pendapatan')>Pendapatan</option>
                <option value="beban"      @selected(old('kelompok')=='beban')>Beban</option>
            </select>
        ...
        <div>
            ...
            <select name="posisi_normal" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                ...
                <option value="debit"  @selected(old('posisi_normal')=='debit')>Debit</option>
                <option value="kredit" @selected(old('posisi_normal')=='kredit')>Kredit</option>
            </select>
        ...
        <div class="col-span-2">
            ...
            <select name="parent_id" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                ...
                <option value="{{ $p->id }}" @selected(old('parent_id') == $p->id)>{{ $p->kode_akun }} · {{ $p->nama_akun }}</option>
                @endforeach
            </select>
