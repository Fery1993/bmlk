<x-app-layout title="Karyawan" breadcrumb="Master data karyawan">

<x-table-wrapper title="Daftar Karyawan" :createRoute="route('master.karyawan.create')" createLabel="Tambah Karyawan">
    ...
    <x-slot:filter>
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('master.karyawan.index') }}" class="text-sm text-gray-400 hover:text-gray-600 flex items-center">Reset</a>
            @endif
        </form>
    </x-slot:filter>
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50">
                <th class="text-left text-xs text-gray-500 font-medium px-5 py-3">NIK</th>
                <th class="text-left text-xs text-gray-500 font-medium px-3 py-3">Nama</th>
                <th class="text-left text-xs text-gray-500 font-medium px-3 py-3">Jabatan</th>
                <th class="text-left text-xs text-gray-500 font-medium px-3 py-3">Telepon</th>
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50 transition">
                <td class="px-5 py-3">
                    <a href="{{ route('master.karyawan.show', $k) }}" class="font-medium text-primary-600 hover:underline">{{ $k->nik }}</a>
                </td>
                <td class="px-3 py-3 text-gray-800">{{ $k->nama }}</td>
                <td class="px-3 py-3 text-gray-600 text-xs">{{ $k->jabatan ?? '—' }}</td>
                <td class="px-3 py-3 text-gray-500 text-xs">{{ $k->telepon ?? '—' }}</td>
                ...
                <td class="px-3 py-3 pr-5 text-center">
                    <div class="flex items-center justify-center gap-2">
                        <a href="{{ route('master.karyawan.show', $k) }}" class="text-gray-400 hover:text-primary-600" title="Detail">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </a>
                        <a href="{{ route('master.karyawan.edit', $k) }}" class="text-gray-400 hover:text-amber-600" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </a>
                        <form method="POST" action="{{ route('master.karyawan.destroy', $k) }}"
